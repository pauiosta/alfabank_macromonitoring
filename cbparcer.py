...
with sqlite3.connect(db_path) as conn:
    # WAL ускоряет одновременную запись/чтение, synchronous=NORMAL снижает задержки сохранения
        ...
        for dataset in datasets:
            ...
            for measure_id in measure_ids:
                ...
                try:
                    ...
                    # Добавляем метаданные для последующей фильтрации в SQLite
                    ...
                    if table_name in saved_tables:
                        df.to_sql(table_name, conn, if_exists="append", index=False)
                    else:
                        df.to_sql(table_name, conn, if_exists="replace", index=False)
                        saved_tables.add(table_name)
...
print("\n📁 Готово! Данные сохранены в cbr_data.sqlite")
